...
function NftCard({ day, claimed, wallet }: { day: number; claimed: boolean; wallet?: string }) {
  ...
  return (
    <article className="relative mx-auto w-full max-w-[430px] overflow-hidden rounded-[28px] bg-[linear-gradient(145deg,#9aa39d_0%,#2d3530_8%,#0b100d_22%,#030604_68%,#59635d_100%)] p-[8px] shadow-[0_36px_85px_rgba(2,14,7,0.42)]">
      <div className="pointer-events-none absolute inset-[2px] rounded-[26px] border border-white/10" />
      <div className="relative overflow-hidden rounded-[21px] border border-[#39443e] bg-[radial-gradient(circle_at_50%_38%,rgba(77,176,34,0.11),transparent_36%),linear-gradient(180deg,#080e0a_0%,#030604_100%)] px-5 pb-5 pt-6 sm:px-7">
        ...
        <div className="pointer-events-none absolute inset-0 opacity-35 [background-image:linear-gradient(rgba(102,209,33,0.045)_1px,transparent_1px),linear-gradient(90deg,rgba(102,209,33,0.045)_1px,transparent_1px)] [background-size:22px_22px]" />
        <div className="pointer-events-none absolute inset-x-8 top-3 h-px bg-gradient-to-r from-transparent via-[#6d796f] to-transparent" />
        <div className="absolute left-4 top-4 h-6 w-16 border-l border-t border-[#707b74]"/><div className="absolute right-4 top-4 h-6 w-16 border-r border-t border-[#707b74]"/><div className="absolute bottom-4 left-4 h-6 w-16 border-b border-l border-[#515c55]"/><div className="absolute bottom-4 right-4 h-6 w-16 border-b border-r border-[#515c55]"/>
        <span className="absolute left-3 top-3 h-1.5 w-1.5 rounded-full bg-[#9ca69f] shadow-[0_0_6px_rgba(255,255,255,.25)]"/><span className="absolute right-3 top-3 h-1.5 w-1.5 rounded-full bg-[#9ca69f]"/><span className="absolute bottom-3 left-3 h-1.5 w-1.5 rounded-full bg-[#606a64]"/><span className="absolute bottom-3 right-3 h-1.5 w-1.5 rounded-full bg-[#606a64]"/>
        ...
        <div className="relative mx-auto mt-4 aspect-square w-full max-w-[330px]">
          <div className="absolute inset-[8%] rounded-full border border-[#314438] bg-[radial-gradient(circle,rgba(65,142,32,0.17),rgba(1,5,3,0.98)_68%)] shadow-[inset_0_0_38px_rgba(67,190,22,0.07),0_0_28px_rgba(70,185,27,0.07)]" />
          {Array.from({length:TOTAL_DAYS},(_,i)=><span key={i} className="absolute left-1/2 top-1/2 h-6 w-2 rounded-sm" style={{transform:`translate(-50%,-50%) rotate(${i*9}deg) translateY(-145px)`,background:i<day?'linear-gradient(180deg,#f2ffd8,#7cf03e 48%,#2c830d)':'linear-gradient(180deg,#323a35,#171c19)',boxShadow:i<day?'0 0 15px rgba(124,240,62,.75)':'inset 0 0 0 1px rgba(255,255,255,.025)'}} />)}
          <div className="absolute inset-[23%] overflow-hidden rounded-full border border-[#415c4b] bg-[#020503] shadow-[0_0_32px_rgba(71,181,25,0.12)]">
            <div className="absolute left-1/2 top-1/2 h-[78%] w-[88%] -translate-x-1/2 -translate-y-1/2 border border-[#4eca23] opacity-80" style={{clipPath:'polygon(50% 0%,100% 100%,0 100%)'}} />
            <img src={arcLogo} alt="Machina robot head" className="absolute left-1/2 top-[2%] z-10 h-auto w-[121%] max-w-none -translate-x-1/2 object-contain drop-shadow-[0_10px_16px_rgba(0,0,0,.62)]" style={{clipPath:'inset(0 0 34% 0)'}} />
          </div>
        </div>
...
export default function CountdownPage() {
  ...
  const download = async () => {
    ...
    const img=new Image(); img.src=arcLogo; await img.decode(); x.drawImage(img,img.naturalWidth*.18,img.naturalHeight*.07,img.naturalWidth*.64,img.naturalHeight*.62,350,405,380,400)
    ...
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <section className="grid gap-7 lg:grid-cols-[minmax(0,1fr)_460px] lg:items-start"><div className="space-y-5">
        <div className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Your progress</p><p className="mt-2 text-4xl font-semibold">{progress.claimedDays.length} / 40</p><p className="mt-1 text-sm text-slate-500">Current tier: {tier(progress.claimedDays.length)}</p></div><div className={`rounded-2xl border px-4 py-3 ${isConnected?'border-emerald-200 bg-emerald-50':'border-slate-200 bg-slate-50'}`}><p className="flex items-center gap-2 text-sm font-semibold"><Wallet size={16}/>{short(address)}</p></div></div>
          <div className="mt-5 border-t border-slate-100 pt-4"><p className="text-sm leading-6 text-slate-600">Claim one NFT per day on Arc Testnet during the 40-day countdown. Your wallet keeps the participation record. Reach <strong className="text-slate-800">20</strong> claims for Initiate, <strong className="text-slate-800">30</strong> for Pioneer, or <strong className="text-slate-800">40/40</strong> for Genesis 40 and <strong className="text-emerald-700">0% Machina service fee</strong> during mainnet week one.</p><p className="mt-2 text-xs text-slate-400">Network and protocol fees still apply.</p></div>
          {!arcSelected&&isConnected&&<button onClick={()=>void switchChainAsync({chainId:ARC_EVM_CHAIN_ID})} className="mt-4 inline-flex items-center gap-2 rounded-xl border border-emerald-300 bg-emerald-50 px-4 py-2.5 text-sm font-semibold text-emerald-900">Switch to Arc Testnet <ExternalLink size={15}/></button>}
        </div>
